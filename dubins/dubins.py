...
from dubins.angle import angle_mod, rot_mat_2d
from dubins.plot import plot_arrow
...
def main():
    ...
    start_x = 12.1  # [m]
    start_y = 15.0  # [m]
    start_yaw = np.deg2rad(80.0)  # [rad]

    end_x = 12.6  # [m]
    end_y = 14.7  # [m]
    end_yaw = np.deg2rad(-140)  # [rad] 

    turning_radius = 0.27 # more turning radius less intermediate points.
    curvature = 1/turning_radius #0.6 # Low curvature more turning radius. High curvature less turning radius
    ...
    path_x, path_y, path_yaw, mode, lengths = plan_dubins_path(start_x,
                                                               ...
                                                               end_y,
                                                               end_yaw,
                                                               curvature, step_size=0.01) # less step size more intermediate points

    print("Path Lengths:", path_x)
    print("Path Lengths:", path_y)
    print("Path Lengths:", len(path_x))
    if show_animation:
